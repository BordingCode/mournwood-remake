...
const grantRelic = (rng) => { const pool = RELIC_POOL.filter((id) => !run.player.relics.includes(id)); if (!pool.length) return null; const id = rng.pick(pool); run.player.relics.push(id); return id; };
...
function title() {
  ...
  const s = screen('title', `
    <div class="titlewrap">
      ...
      <p class="m1note">Milestone 2 · Houndmaster · Wealdedge — the branching wood</p>
    </div>`);
  s.querySelector('.newrun').onclick = () => { audio.resume(); clearSave(); run = makeRun({ hunterId: 'houndmaster' }); saveRun(run); showMap(); };
...
function startCombat(node) {
  ...
  window.__mw.screen = new CombatScreen(host, {
    ...
    hound: { name: run.hound.name, maxHp: run.hound.maxHp, atk: run.hound.atk },
    enemyIds: ids,
  }, audio, (result, combat) => {
...
function victory() {
  ...
  const s = screen('over win', `<div class="titlewrap">
    ...
    <p class="m1note">Milestone 2 complete: full run loop — map, camps, trader, relics, curses, events, save/resume.<br>Next (M4): the Sloughfen &amp; the Blackheart, and (M3) the Assassin, Tinker &amp; Pacts.</p>
